...
export function Accommodation() {
  ...
  useEffect(() => {
    const dcityPrice = cities.find(c => c.id === Number(cityId)) ? cities.find(c => c.id === Number(cityId)).accommodation : 0;
    ...
    if (nAdults > 0)
      totalPrice = totalPrice + (nAdults * 40);
    if (nKids)
      totalPrice = totalPrice + (nKids * 25);
    if (nElderly)
      totalPrice = totalPrice + (nElderly * 25);

    setTotal(totalPrice);
  }, [cityId, isRoundTrip, nAdults, nKids, nElderly, backDate, goingDate, nRooms]);
  ...
  return (
    <Box>
      <Heading size="lg" marginBottom="3">Alugar Hospedagem</Heading>
          ...
          <Box>
            <Stack direction="column">
              ...
              <FormControl id="adult-number" isRequired>
                  ...
                  <NumberInput min={0} value={nRooms} onChange={value => setNRooms(value)}>
                    <NumberInputField />
                    <NumberInputStepper>
